...
<head>
  ...
  <title>HRM Verzatec | Autorización de Objetivos Ejecutivos PADE</title>
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    if Tvl = 1 then
        response.redirect "Hrm_PadePAAutObj.asp?Mdl=" & Mdl & "&Trn=" & Trn & "&Sid=" & Sid & "&Ejr=" & Ejr & "&Eid=" & Eid & "&Nem=" & Nem & "&Cid=" & Cid
    end if
    ...
    sqlEml = "select Emp_Email, Emp_NombreCompleto from HRM10220 where Emp_EmpleadoID = '"& Eid &"' "
    set rsEml = dbconn.execute(sqlEml)
    if not rsEml.bof and not rsEml.eof then
        Aml = trim(rsEml("Emp_Email"))
        Enm = trim(rsEml("Emp_NombreCompleto"))
    else
        Aml = ""
        Enm = ""
    end if
...
'        response.write("</br>")
        ...
        TskScr = "Hrm_PadeAprObjE.asp?Mdl=" & Mdl &  "&Trn=" & Trn & "&Ejr=" & Ejr & "&Nem=" & Eid & "&Emb=" & Enm & "&Dsc=" & Dsc & "&Cid=" & Cid
        TskDsc = Dsc & " - " & TskTx2
    ...
    <section class="content">
      ...
      <div class="box box-success">
        ...
        <div class="box box-primary">
            <form action="Hrm_PadePAAutObjE.asp?Mdl=<%= Mdl & "&Trn=" & Trn & "&Ejr=" & Ejr & "&Dsc=" & Dsc & "&Sid=" & Sid & "&Eid=" & Eid & "&Nem=" & Nem & "&Nmb=" & Nmb & "&Cid=" & Cid %>" name="form1" method="post">
                <!-- form start -->
                <div class="box-body">
                <%
                EstNeg = "EST" & Ejr
                sqlObj = "select a.*, b.Trd_Texto                        " & _
                         "from HRM10531 a, HRM10002 b                    " & _
                         "where a.Pod_EstrategiaID  = b.Trd_ElementoID   " & _
                         "and   b.Trd_TransaccionID = '"& EstNeg &"'     " & _
                         "and   b.Trd_IdiomaID      = '"& Lng    &"'     " & _
                         "and   a.Pod_CorporativoID = '5000'             " & _
                         "and   a.Pod_CompaniaID    = '"& Cid    &"'     " & _
                         "and   a.Pod_EmpleadoID    = '"& Nem    &"'     " & _
                         "and   a.Pod_Ejercicio     =  "& Ejr    &"      " & _
                         "and   a.Pod_Tipo          = 1                  " & _
                         "order by a.Pod_Partida                         "
