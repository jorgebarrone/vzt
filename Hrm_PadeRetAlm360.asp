...
'    <!-- Validamos que exista un Signature (1 = Jefe, 2 = Colaborador) -->'
     if Sgn > "" then
         if Sgn = "2" then 'El Signature es del Empleado'
'            <!-- Validamos que el Empleado tenga Autoevaluacion de lo contrario creamos el registro para que conteste la autoevaluación -->'
             sqlAuv = "select M36_EvaluadorID from HRM10540 " & _
                      "where M36_Ejercicio   =  "& Ejr &"   " & _
                      "and   M36_EmpleadoID  = '"& Eid &"'  " & _
                      "and   M36_EvaluadorID = '"& Eid &"'  "
             set rsAuv = dbconn.execute(sqlAuv)
             if rsAuv.bof and rsAuv.eof then 'No existe la autoevaluación'
                 'Creamos el registro de la autoevaluación'
