...
<head>
  ...
  <title>HRM Verzatec | Areas </title>
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <%
    sqlTrn = "select Trd_ElementoID, Trd_Texto from HRM10002 where Trd_TransaccionID = 'DisOrg0400' and Trd_IdiomaID = '"& Lng &"'  "
    ...
    if Bta = "A" then
        ...
        sqlexs = "select Are_CompaniaID from HRM10204      " & _
                 "where  Are_CompaniaID  = '" & Cpy & "'   " & _
                 "and    Are_AreaID      = '" & Did & "'   "
        set rs = dbconn.Execute (sqlexs)
        if rs.eof and rs.bof then
            sqlins = "insert into HRM10204                  " & _
                     "values ('" & Cpy & "', '" & Did & "', " & _
                     "        '" & Ddv & "')                "
     ...
     if Btm = "M" then
	    for e = 1 to Ind
		    if request.Form("elm"&e) = "on" then
	 		    sqldel = "delete from HRM10204                                      " & _
			  		     "where Are_CompaniaID  = '" & Request.Form("Lcpy"&e) & "'  " & _
			  		     "and   Are_AreaID      = '" & Request.Form("Ldid"&e) & "'  "
    ...
    <section class="content">
        ...
        <!-- Agregar Divisiones -->
        <form action="Hrm_Areas.asp?Mdl=<%= Mdl & "&Trn=" & Trn & "&Sid=" & Sid %>" name="form1" method="post">
        ...
        sqlLis = "select a.*,  b.Cpy_RazonSocial             " & _
                 "from HRM10204 a, HRM10201 b                " & _
                 "where a.Are_CompaniaID = b.Cpy_CompaniaID  " & _
                 "order by a.Are_CompaniaID                  "
            
        set rs = dbconn.Execute(sqlLis)
        'response.write(sqlLis)
        if not rs.bof and not rs.eof then
            %>
            <form action="Hrm_Areas.asp?Mdl=<%= Mdl & "&Trn=" & Trn & "&Sid=" & Sid %>" name="form2" method="post">
                        ...
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                ...
                                do while not rs.eof
                                    ...
                                    <tr>
                                        <td>
                                            <input type="hidden" name="<%= "Lcpy"&i %>" id="<%= "Lcpy"&i %>" value="<%= trim(rs("Are_CompaniaID")) %>" />
                                            <input type="hidden" name="<%= "Ldid"&i %>" id="<%= "Ldid"&i %>" value="<%= trim(rs("Are_AreaID")) %>" />
                                            <%= trim(rs("Are_CompaniaID")) %>
                                        </td>
                                        <td><%= trim(rs("Cpy_RazonSocial")) %></td>
                                        <td><%= trim(rs("Are_AreaID")) %></td>
                                        <td><%= trim(rs("Are_Descripcion")) %></td>
