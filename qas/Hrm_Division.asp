...
<head>
  ...
  <title>HRM Verzatec | Division </title>
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <%
    sqlTrn = "select Trd_ElementoID, Trd_Texto from HRM10002 where Trd_TransaccionID = 'DisOrg0200' and Trd_IdiomaID = '"& Lng &"'  "
    ...
    if Bta = "A" then
        ...
        sqlexs = "select Div_CompaniaID from HRM10202     " & _
                 "where  Div_CompaniaID = '" & Cpy & "'   " & _
                 "and    Div_DivisionID = '" & Did & "'   "
        set rs = dbconn.Execute (sqlexs)
        if rs.eof and rs.bof then
            sqlins = "insert into HRM10202                  " & _
                     "values ('" & Cpy & "', '" & Did & "', " & _
                     "        '" & Ddv & "')                "
     ...
     if Btm = "M" then
	    for e = 1 to Ind
		    if request.Form("elm"&e) = "on" then
	 		    sqldel = "delete from HRM10202                                     " & _
			  		     "where Div_CompaniaID = '" & Request.Form("Lcpy"&e) & "'  " & _
			  		     "and   Div_DivisionID = '" & Request.Form("Ldid"&e) & "'  "
    ...
    <section class="content">
        ...
        <!-- Agregar Divisiones -->
        <form action="Hrm_Division.asp?Mdl=<%= Mdl & "&Trn=" & Trn & "&Sid=" & Sid %>" name="form1" method="post">
        ...
        sqlLis = "select a.*,  b.Cpy_RazonSocial             " & _
                 "from HRM10202 a, HRM10201 b                " & _
                 "where a.Div_CompaniaID = b.Cpy_CompaniaID  " & _
                 "order by a.Div_CompaniaID                  "
            
        set rs = dbconn.Execute(sqlLis)
        'response.write(sqlLis)
        if not rs.bof and not rs.eof then
            %>
            <form action="Hrm_Division.asp?Mdl=<%= Mdl & "&Trn=" & Trn & "&Sid=" & Sid %>" name="form2" method="post">
                        ...
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                ...
                                do while not rs.eof
                                    ...
                                    <tr>
                                        <td>
                                            <input type="hidden" name="<%= "Lcpy"&i %>" id="<%= "Lcpy"&i %>" value="<%= trim(rs("Div_CompaniaID")) %>" />
                                            <input type="hidden" name="<%= "Ldid"&i %>" id="<%= "Ldid"&i %>" value="<%= trim(rs("Div_DivisionID")) %>" />
                                            <%= trim(rs("Div_CompaniaID")) %>
                                        </td>
                                        <td><%= trim(rs("Cpy_RazonSocial")) %></td>
                                        <td><%= trim(rs("Div_DivisionID")) %></td>
                                        <td><%= trim(rs("Div_Descripcion")) %></td>
