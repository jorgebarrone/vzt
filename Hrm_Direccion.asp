...
<head>
  ...
  <title>HRM Verzatec | Direccion </title>
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <%
    sqlTrn = "select Trd_ElementoID, Trd_Texto from HRM10002 where Trd_TransaccionID = 'DisOrg0300' and Trd_IdiomaID = '"& Lng &"'  "
    ...
    if Bta = "A" then
        ...
        sqlexs = "select Dir_CompaniaID from HRM10203      " & _
                 "where  Dir_CompaniaID  = '" & Cpy & "'   " & _
                 "and    Dir_DireccionID = '" & Did & "'   "
        set rs = dbconn.Execute (sqlexs)
        if rs.eof and rs.bof then
            sqlins = "insert into HRM10203                  " & _
                     "values ('" & Cpy & "', '" & Did & "', " & _
                     "        '" & Ddv & "')                "
     ...
     if Btm = "M" then
	    for e = 1 to Ind
		    if request.Form("elm"&e) = "on" then
	 		    sqldel = "delete from HRM10203                                      " & _
			  		     "where Dir_CompaniaID  = '" & Request.Form("Lcpy"&e) & "'  " & _
			  		     "and   Dir_DireccionID = '" & Request.Form("Ldid"&e) & "'  "
    ...
    <section class="content">
        ...
        <!-- Agregar Divisiones -->
        <form action="Hrm_Direccion.asp?Mdl=<%= Mdl & "&Trn=" & Trn & "&Sid=" & Sid %>" name="form1" method="post">
        ...
        sqlLis = "select a.*,  b.Cpy_RazonSocial             " & _
                 "from HRM10203 a, HRM10201 b                " & _
                 "where a.Dir_CompaniaID = b.Cpy_CompaniaID  " & _
                 "order by a.Dir_CompaniaID                  "
            
        set rs = dbconn.Execute(sqlLis)
        'response.write(sqlLis)
        if not rs.bof and not rs.eof then
            %>
            <form action="Hrm_Direccion.asp?Mdl=<%= Mdl & "&Trn=" & Trn & "&Sid=" & Sid %>" name="form2" method="post">
                        ...
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                ...
                                do while not rs.eof
                                    ...
                                    <tr>
                                        <td>
                                            <input type="hidden" name="<%= "Lcpy"&i %>" id="<%= "Lcpy"&i %>" value="<%= trim(rs("Dir_CompaniaID")) %>" />
                                            <input type="hidden" name="<%= "Ldid"&i %>" id="<%= "Ldid"&i %>" value="<%= trim(rs("Dir_DireccionID")) %>" />
                                            <%= trim(rs("Dir_CompaniaID")) %>
                                        </td>
                                        <td><%= trim(rs("Cpy_RazonSocial")) %></td>
                                        <td><%= trim(rs("Dir_DireccionID")) %></td>
                                        <td><%= trim(rs("Dir_Descripcion")) %></td>
