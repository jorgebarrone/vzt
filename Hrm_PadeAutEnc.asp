...
'                       <!--  Validamos si la Matriz del Solicitante cuenta con el Minimo de Evaluadores (sin contar la AutoEvaluacion) de lo contrario pondremos en Rojo el Semaforo  -->
                        sqlMev = "select isnull(count(M36_EvaluadorID),0) as Tev  " & _
                                 "from HRM10540                                   " & _
                                 "where M36_CorporativoID = '5000'                " & _
                                 "and   M36_Ejercicio     =  "& Ejr &"            " & _
                                 "and   M36_EmpleadoID    = '"& Nemp &"'          " & _
                                 "and   M36_Estatus IN(1,3,4)                     " & _
                                 "and   M36_EvaluadorTipo > 0                     "
                        set rsMev = dbconn.execute(sqlMev)
                        if not rsMev.bof and not rsMev.eof then
                            if Cint(rsMev("Tev")) < 3 then
...
'                        response.write(sqlupd)
    ...
    <section class="content">
        ...
        <div class="box box-success">
            ...
            <form action="Hrm_PadeAutEnc.asp?Mdl=<%= Mdl & "&Trn=" & Trn & "&Ejr=" & Ejr & "&Dsc=" & Dsc & "&Sid=" & Sid & "&Eid=" & Eid & "&Nmb=" & Unm %>" name="form1" method="post">
            <%
