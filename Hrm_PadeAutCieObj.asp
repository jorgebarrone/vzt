...
<head>
  ...
  <title>HRM Verzatec | Autoriza Cierre de Objetivos Empleados PADE</title>
...
'    response.write("HdrNim : " & HdrNim & ", HdrNmb : " & HdrNmb & ", HdrNml : " & HdrNml & ", HdrNpd : " & HdrNpd)
    ...
    sqlAml = "select Emp_Email from HRM10220 where Emp_EmpleadoID = '"& Eid &"' "
    set rsAml = dbconn.execute(sqlAml)
    if not rsAml.bof and not rsAml.eof then
        Aml = trim(rsAml("Emp_Email"))
    else
        Aml = ""
    end if
...
'        response.write("</br>")
        ...
        TskScr = "Hrm_PadeCieObj.asp?Mdl=" & Mdl &  "&Trn=" & Trn & "&Ejr=" & Ejr & "&Nem=" & Nem & "&Nmb=" & Unm & "&Dsc=" & Dsc & "&Cid=" & Cid
        TskDsc = Dsc & " - " & TskTx5
        ...
        if EnvEml = 1 then
		    ...
		    if trim(Eml) <> "" then
                ...
                msgtext = Unm & "  " & EmMs05 & "</br></br><p>" & lnki & "</br></br><p>" & lnke 
                myMail.From = Emc
                myMail.To = HdrNml
'                myMail.Cc = Aml
...
'<!-- -->
        ...
        if EnvEml = 1 then
		    ...
		    if trim(Eml) <> "" then
                Set myMail=CreateObject("CDO.Message")
                myMail.Subject = TskDsc
                msgtext = Unm & "  " & EmMs06 & "</br></br><p>" & lnki & "</br></br><p>" & lnke 
                myMail.From = Emc
                myMail.To = HdrNml
                'myMail.Cc = Aml
                myMail.Bcc = "jorge.barron@eryseg.com"
    ...
    <section class="content">
        <div class="box box-success">
            <form action="Hrm_PadeAutCieObj.asp?Mdl=<%= Mdl & "&Trn=" & Trn & "&Ejr=" & Ejr & "&Dsc=" & Dsc & "&Sid=" & Sid & "&Eid=" & Eid & "&Nem=" & Nem & "&Nmb=" & Nmb & "&Dsc=" & Dsc & "&Cid=" & Cid %>" name="form1" method="post">
                <div class="box-header with-border">
                    <h3 class="box-title">
                        <img src="<%= HdrNim %>" style="height: 45px;" alt="<%= HdrNmb %>">
                        <%= Nem & " - " & HdrNmb & " - " & HdrNpd %><label style=" font-weight: bold">&nbsp;&nbsp;&nbsp;TOTAL:&nbsp;&nbsp;&nbsp;<%= formatnumber(Cal,2) %></label>
                    </h3>
                </div>
                <!-- form start -->
                <div class="box-body">
                ...
                if not rsO.bof and not rsO.eof then
                    ...
                    <div class="box box-primary">
                        <form action="Hrm_PadeAutCieObj.asp?Mdl=<%= Mdl & "&Trn=" & Trn & "&Ejr=" & Ejr & "&Dsc=" & Dsc & "&Sid=" & Sid & "&Eid=" & Eid & "&Nem=" & Nem & "&Nmb=" & Nmb & "&Dsc=" & Dsc & "&Cid=" & Cid %>" name="form1a" method="post">
<!--
                                        ...
                                        if not rsS.bof and not rsS.eof then
                                            ...
                                            <tr>
                                                <td>
                                                    ...
                                                    <a href="Hrm_PadeCnsCieObj1.asp?Mdl=<%= Mdl & "&Trn=" & Trn & "&Sid=" & Sid & "&Eid=" & Nem & "&Ejr=" & Eja & "&Dsc=" & Dsc & "&Ptd=" & rsO("Pod_Partida") %>">
                                                        <%= rsO("Pod_Ponderacion") %>
                                                    </a>
                                                </td>
                                                <td>
                                            ...
                                            do while not rsS.eof
                                                ...
                                                <tr>
                                                    <td style="text-align: right">
                                                        ...
                                                        <a href="Hrm_PadeCnsCieObj1.asp?Mdl=<%= Mdl & "&Trn=" & Trn & "&Sid=" & Sid & "&Eid=" & Nem & "&Ejr=" & Eja & "&Dsc=" & Dsc & "&Ptd=" & rsS("Pod_Partida") %>">
                                                            <%= rsS("Pod_Ponderacion") %>
                                                        </a>
                                                    </td>
                                                    <td>&nbsp;</td>
